...
if platform.system() == "Windows":
    ...
    if not os.path.isfile(xterm):
        ...
        xbmcgui.Dialog().ok(__localizedMessages__(32013),
                            __localizedMessages__(32014) + " " +
                            __localizedMessages__(32015) + "\n" +
                            __localizedMessages__(32003) + xterm)
        # Log the error
        log("xterm executable was not found on this specified location: " + xterm, xbmc.LOGERROR)
    launch = [xterm, "-fullscreen" ,"-e", executable]

# When you choose an executable from Kodi when changing Addon Settings,
# Kodi forces you to select an executable that exists, but we want to validate if the executable exists anyways,
# just in case the default value doesn't work or the settings were manually changed
if not os.path.isfile(executable):
    # This message prints something like: EmulationStation executable was not found, go to Addon-Configure and change it
    xbmcgui.Dialog().ok(__localizedMessages__(32000),
                        __localizedMessages__(32001) + " " +
                        __localizedMessages__(32002) + "\n" +
                        __localizedMessages__(32003) + executable)
    ...
    if __addon__.getSetting('powersaving') == "true":
        ...
        # Before reenabling powersavings, send Down key to Kodi to wake up.
        # Otherwise computer goes to sleep as soon as parameter is set
